...
internal class Day18 : IDay
{
    ...
    private void StickCorners(bool[,] buffer)
    {
        buffer[0, 0] = true;
        buffer[_height - 1, 0] = true;
        buffer[0, _width - 1] = true;
        buffer[_height - 1, _width - 1] = true;
    }
    ...
    public void Process(string inputFile)
    {
        ...
        StickCorners(FrontBuffer);

        // iterate
        for (int i = 0; i < _iterations; i++)
        {
            Iterate();
            StickCorners(BackBuffer);
            SwapBuffers();
        }
